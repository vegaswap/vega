...
def test_claim(accounts):
    ...
    b = token.balanceOf(accounts[1])
    assert b == 32
    # assert bucket.totalClaimAmount() == 1000
    assert bucket.openClaimAmount() == 1000-b

    chain.sleep(day * 12)
    tx = bucket.vestClaimMax(accounts[1], {"from": accounts[1]})
    b2 = token.balanceOf(accounts[1])
    assert b2 == 200
    assert bucket.openClaimAmount() == 1000-b2

    with brownie.reverts():
        tx = bucket.vestClaimMax(accounts[2], {"from": accounts[1]})

    tx = bucket.vestClaimMax(accounts[2], {"from": accounts[2]})
    b3 = token.balanceOf(accounts[2])
    assert b3 == 300
    assert bucket.openClaimAmount() == 1000-b2-b3

    tx = bucket.vestClaimMax(accounts[3], {"from": accounts[3]})
    b4 = token.balanceOf(accounts[3])
    assert b4 == 500
    assert bucket.openClaimAmount() == 1000-b2-b3-b4
